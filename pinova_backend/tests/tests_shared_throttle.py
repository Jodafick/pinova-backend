...
from pinova_backend.config.cache import build_caches_config, warn_if_locmem_in_production
from pinova_backend.security.throttling import AnonIPRateThrottle

SHARED_LOC = 'pinova-shared-throttle-test'
...
class CacheConfigTests(SimpleTestCase):
    ...
    def test_warn_if_locmem_in_production_logs_warning(self):
        with self.assertLogs('pinova.cache', level='WARNING') as captured:
    ...
    def test_no_warning_when_redis_or_debug(self):
        with self.assertNoLogs('pinova.cache', level='WARNING'):
...
class RatelimitExceptionHandlerTests(SimpleTestCase):
    def test_pinova_exception_handler_maps_ratelimited_to_429(self):
        from pinova_backend.core.exceptions import pinova_exception_handler
        ...
        response = pinova_exception_handler(Ratelimited(), {})
        self.assertIsNotNone(response)
        self.assertEqual(response.status_code, 429)
        self.assertEqual(response.data['code'], 'pinova_rate_limited')
