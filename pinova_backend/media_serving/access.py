...
from pins.models import Comment, Pin, PinVariant, Topic
from pins.visibility import (
...
logger = logging.getLogger('pinova.media.access')
...
class _ViewerRequest:
    """Minimal request pour réutiliser ``pin_is_visible_for_request``."""
...
def resolve_pin_for_media_path(relative_path: str) -> Pin | None:
    rel = normalize_relative_media_path(relative_path)
    qs = Pin.objects.select_related('author', 'author__profile')
    if rel.startswith('story_videos/'):
        return qs.filter(story_video=rel).first()
    if rel.startswith('pins/variants/'):
        variant = (
            PinVariant.objects.filter(image=rel)
...
def resolve_media_resource(relative_path: str) -> MediaResource | None:
    rel = normalize_relative_media_path(relative_path)
    pin = resolve_pin_for_media_path(rel)
    if pin:
        return MediaResource(kind='pin', relative_path=rel, pin=pin)
    ...
    if comment:
        return MediaResource(kind='comment', relative_path=rel, comment=comment, pin=comment.pin)
...
def media_visible_for_viewer(resource: MediaResource, request: HttpRequest, *, uid: int | None = None) -> bool:
    ...
    if resource.kind == 'pin' and resource.pin:
        return pin_is_visible_for_request(resource.pin, viewer_req)
    if resource.kind == 'comment' and resource.pin:
        return pin_is_visible_for_request(resource.pin, viewer_req)
    if resource.kind == 'profile' and resource.profile:
...
def evaluate_media_access(request: HttpRequest, relative_path: str) -> tuple[bool, str, MediaResource | None]:
    ...
    if policy == 'public':
        if resource and resource.kind in ('pin', 'comment') and resource.pin:
            if not pin_is_visible_for_request(resource.pin, request):
...
def media_requires_signed_url(relative_path: str, *, pin: Pin | None = None, profile: Profile | None = None) -> bool:
    rel = normalize_relative_media_path(relative_path)
    if pin is None:
        pin = resolve_pin_for_media_path(rel)
    if pin is not None:
        return pin_media_requires_signed_url(pin)
    if profile is None:
        profile = resolve_profile_for_media_path(rel)
