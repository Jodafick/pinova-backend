...
from pinova_backend.media_serving.access import evaluate_media_access, safe_join_media_root
...
def serve_media(request, path):
    if not getattr(request, 'pinova_media_access_granted', False):
        allowed, reason, _resource = evaluate_media_access(request, path)
        if not allowed:
            return JsonResponse({'error': 'Forbidden'}, status=403)
        request.pinova_media_access_granted = True
        request.pinova_media_access_reason = reason
