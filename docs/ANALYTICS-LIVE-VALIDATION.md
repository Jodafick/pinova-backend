# Validation analytics live PINOVA — PostHog EU

**Date** : 6 juin 2026  
**Dashboard** : `PINOVA — Business`  
...
1. Crée ou réutilise le dashboard **PINOVA — Business**
...
| `first_pin_published` | Web, mobile | Premier pin/story publié (`trackOnce`) |
...
|---------|-------------------|---------|
| Acquisition — landing → first pin | Où perd-on les visiteurs ? | 4 étapes, fenêtre 14 j, breakdown `signup_channel` |
...
| Activation < 24 h | Time-to-value | `register_completed` → `first_pin_published` ≤ 1 jour |
...
Utilisateurs ayant publié un premier pin dans les 24 h après inscription :
...
## 6. Pourquoi les users partent — funnel abandon register → first pin

### Funnel PostHog (copier dans Insights → Funnels)

```
register_completed → onboarding_completed → first_pin_published
```
...
LEFT JOIN (
  SELECT distinct_id FROM events WHERE event = 'first_pin_published'
) fp ON reg.distinct_id = fp.distinct_id
```
...
| Onboarding OK mais pas de first pin | Peur de publier, UX create | Push create pin, templates, tutoriel |
...
| Activation 24 h < 20 % | Time-to-value lent | Email J0 « publie ton premier pin » |
...
onboarding_started → onboarding_step_completed (step_id=interests)
  ...
  → onboarding_completed → first_pin_published
```
...
Test Playwright : `PINOVA-FRONTEND/e2e/analytics-consent.spec.ts`

| Choix bannière | `pinova_analytics_consent` | `pinova_analytics_opt_out` | Capture PostHog |
|----------------|---------------------------|---------------------------|-----------------|
...
- [ ] Alertes : chute `first_pin_published`, spike `guest_action_blocked`
...
| Fichier | Rôle |
|---------|------|
| `pinova-backend/scripts/setup_posthog_business_dashboard.py` | Déploiement dashboard |
| `pinova-backend/scripts/verify_posthog_business_events.py` | Audit events |
| `docs/posthog/business-dashboard.json` | Spec insights |
| `docs/BUSINESS-METRICS.md` | Référence KPIs |
| `packages/pinova-shared/src/analytics/` | Schéma événements |
| `PINOVA-FRONTEND/e2e/analytics-consent.spec.ts` | Test consent → PostHog |
