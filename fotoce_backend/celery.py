"""
Application Celery Fotoce — broker Redis, beat planifié, retry exponentiel.
...
# |----------------------------------|------------------------------------|--------------------|
# | pins-send-weekly-pro-digest      | fotos.send_weekly_pro_digest        | lun 09:00          |
# | pins-purge-ephemeral-stories     | fotos.purge_expired_ephemeral_stories | */h :15          |
# | pins-publish-scheduled           | fotos.publish_scheduled_fotos        | */5 min            |
...
app.conf.beat_schedule = {
    'pins-send-weekly-pro-digest': {
        'task': 'fotos.send_weekly_pro_digest',
        'schedule': crontab(hour=9, minute=0, day_of_week=1),
        'options': {'expires': 3600},
    },
    'pins-purge-ephemeral-stories': {
        'task': 'fotos.purge_expired_ephemeral_stories',
        'schedule': crontab(minute=15),
        'options': {'expires': 3000},
    },
    'pins-publish-scheduled': {
        'task': 'fotos.publish_scheduled_fotos',
