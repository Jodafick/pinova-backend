...
class HomeViewTests(SimpleTestCase):
    def test_home_json_with_accept_header(self):
        ...
        data = response.json()
        self.assertEqual(data['service'], 'fotoce-api')
        self.assertEqual(data['status'], 'ok')
        self.assertIn('health', data['endpoints'])

    def test_home_html_by_default(self):
        with patch('fotoce_backend.health.views._collect_checks', return_value=_mock_checks()):
            response = self.client.get(reverse('home'), HTTP_ACCEPT='text/html')
        self.assertEqual(response.status_code, 200)
        self.assertIn('text/html', response['Content-Type'])
        self.assertContains(response, 'Fotoce API')
...
@override_settings(
    CORS_ALLOWED_ORIGINS=['https://fotoce-three.vercel.app'],
...
class CorsRequestIdHeaderTests(SimpleTestCase):
    def test_preflight_allows_x_request_id(self):
        response = self.client.options(
            reverse('health'),
            HTTP_ORIGIN='https://fotoce-three.vercel.app',
