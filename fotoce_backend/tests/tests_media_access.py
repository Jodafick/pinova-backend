...
from fotoce_backend.media_serving.access import build_media_signature
...
class MediaAccessTests(APITestCase):
    def setUp(self):
        self.author = User.objects.create_user('author', 'author@example.com', 'Fotoce2026!')
        self.other = User.objects.create_user('other', 'other@example.com', 'Fotoce2026!')

    def _create_pin(self, *, visibility: str, is_story: bool = False) -> Foto:
    ...
    def test_private_foto_media_without_token_forbidden(self):
        foto = self._create_pin(visibility=Foto.VISIBILITY_PRIVATE)
        response = self.client.get(f'/media/{pin.image.name}')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_private_foto_media_with_valid_signed_url_ok(self):
        foto = self._create_pin(visibility=Foto.VISIBILITY_PRIVATE)
        response = self.client.get(self._signed_url(pin.image.name, self.author.id))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_private_foto_signed_url_wrong_viewer_forbidden(self):
        foto = self._create_pin(visibility=Foto.VISIBILITY_PRIVATE)
        response = self.client.get(self._signed_url(pin.image.name, self.other.id))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_public_foto_media_without_token_ok(self):
        foto = self._create_pin(visibility=Foto.VISIBILITY_PUBLIC)
        response = self.client.get(f'/media/{pin.image.name}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_story_media_requires_signed_url(self):
        foto = self._create_pin(visibility=Foto.VISIBILITY_PUBLIC, is_story=True)
        response = self.client.get(f'/media/{pin.image.name}')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        response = self.client.get(self._signed_url(pin.image.name, self.author.id))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
