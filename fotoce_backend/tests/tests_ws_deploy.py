...
class DeployChannelLayerCheckTests(SimpleTestCase):
    def test_inmemory_rejected_in_production(self):
        from fotoce_backend.core.checks import check_redis_channel_layer_in_production

        errors = check_redis_channel_layer_in_production(None)
        ids = [e.id for e in errors]
        self.assertIn('fotoce.E003', ids)
    ...
    @patch('fotoce_backend.core.checks.redis_url_from_env', return_value='redis://localhost:6379/0')
    ...
    @patch('fotoce_backend.core.checks.redis_url_from_env', return_value='')
    def test_missing_redis_url_fails_deploy_check(self, _mock_redis):
        from fotoce_backend.core.checks import check_redis_channel_layer_in_production

        errors = check_redis_channel_layer_in_production(None)
        self.assertTrue(any(e.id == 'fotoce.E004' for e in errors))
...
class DeployRedisCacheCheckTests(SimpleTestCase):
    ...
    def test_locmem_cache_rejected_in_production(self, _mock_redis):
        from fotoce_backend.core.checks import check_redis_cache_in_production

        errors = check_redis_cache_in_production(None)
        ids = [e.id for e in errors]
        self.assertIn('fotoce.E005', ids)
    ...
    def test_missing_redis_url_fails_cache_deploy_check(self, _mock_redis):
        ...
        errors = check_redis_cache_in_production(None)
        self.assertTrue(any(e.id == 'fotoce.E006' for e in errors))
