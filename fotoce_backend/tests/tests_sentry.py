...
class SentryScrubTests(SimpleTestCase):
    def test_scrub_tokens_and_email(self):
        from fotoce_backend.observability.sentry import scrub_sentry_event
...
class SentryApiTimingMiddlewareTests(SimpleTestCase):
    def test_slow_api_triggers_capture(self):
        from fotoce_backend.middleware.sentry import SentryApiTimingMiddleware
        ...
        with patch('fotoce_backend.middleware.sentry.capture_slow_api') as mock_capture:
            with patch('fotoce_backend.middleware.sentry.time.perf_counter', side_effect=[0, 1.5]):
    ...
    def test_fast_api_skipped(self):
        ...
        with patch('fotoce_backend.middleware.sentry.capture_slow_api') as mock_capture:
            with patch('fotoce_backend.middleware.sentry.time.perf_counter', side_effect=[0, 0.05]):
