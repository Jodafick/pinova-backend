...
Exécution :
  cd fotoce-backend && python manage.py test fotoce_backend.tests_failure_modes -v 2
"""
...
class RedisReadyFailureTests(SimpleTestCase):
    def test_not_ready_when_redis_down(self):
        with patch(
            'fotoce_backend.health.views._collect_checks',
...
class RedisChaosHealthTests(TestCase):
    ...
    @patch('fotoce_backend.health.views._broker_ping', return_value=(True, 'ok'))
    @patch('fotoce_backend.health.views._check_fedapay', return_value={'ok': True, 'detail': 'ok', 'latency_ms': 1.0})
...
class RedisGracefulDegradeTests(SimpleTestCase):
    ...
    def test_ws_connection_fail_open_when_cache_raises(self):
        ...
        with patch('fotoce_backend.websocket.ratelimit.cache.get', side_effect=ConnectionError('redis down')):
...
class CriticalBeatScheduleTests(SimpleTestCase):
    def test_all_critical_beat_tasks_registered(self):
        ...
        import fotos.search.tasks  # noqa: F401
...
class SmtpDownApiTests(APITestCase):
    def test_register_returns_email_delivery_unavailable(self):
        payload = {
            ...
            'password1': 'Str0ngPass!fotoce',
            'password2': 'Str0ngPass!fotoce',
        }
        with patch('accounts.serializers.send_fotoce_mail', side_effect=EmailDeliveryUnavailable('smtp down')):
    ...
    def test_resend_otp_returns_email_delivery_unavailable(self):
        ...
        user = User.objects.create_user('otpuser', 'otpuser@example.com', 'Str0ngPass!fotoce')
        EmailAddress.objects.create(user=user, email=user.email, verified=False, primary=True)
        EmailOTP.objects.create(user=user, expires_at=timezone.now() + timedelta(minutes=10))
        with patch('accounts.views.send_fotoce_mail', side_effect=EmailDeliveryUnavailable('smtp down')):
...
class ExportEmailFailureTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user('exportfail', 'exportfail@example.com', 'Str0ngPass!fotoce')
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)

    @patch('accounts.gdpr_views.send_fotoce_mail', side_effect=EmailDeliveryUnavailable('smtp down'))
...
class FedapayCircuitOpenCheckoutTests(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user('payer', 'payer@example.com', 'Str0ngPass!fotoce')
        self.client.force_authenticate(user=self.user)
