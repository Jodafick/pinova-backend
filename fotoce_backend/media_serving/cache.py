...
def build_versioned_media_url(request, field_file) -> str:
    """URL absolue + ``cv`` ; signature HMAC si le média n'est pas public."""
    if not request or not field_file or not getattr(field_file, 'name', None):
        return ''
    from fotoce_backend.media_serving.access import (
        append_signed_query,
        app_media_url,
        media_requires_signed_url,
    )
    ...
    if getattr(settings, 'USE_CLOUDINARY_MEDIA', False):
        from fotoce_backend.media_serving.cloudinary_urls import cloudinary_delivery_url
...
def append_version_using_media_path(absolute_url: str, *, relative_under_media: str | None) -> str:
    ...
    url = append_cache_version(absolute_url, tok)
    rel = (relative_under_media or '').replace('\\', '/').lstrip('/')
    if rel:
        from fotoce_backend.media_serving.access import append_signed_query, media_requires_signed_url

        if media_requires_signed_url(rel):
            return append_signed_query(url, rel, 0)
    return url
