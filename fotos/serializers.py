...
def _secure_foto_image_upload(value, request):
    if not value:
        return value
...
class BoardSerializer(serializers.ModelSerializer):
    ...
    def get_preview_images(self, obj):
        from .visibility import foto_is_visible_for_request
        ...
        urls = []
        for row in rows:
            foto = getattr(row, 'pin', None)
            if not foto or not request or not foto_is_visible_for_request(pin, request):
                continue
...
class FotoProvenanceEventSerializer(serializers.ModelSerializer):
    actor_username = serializers.CharField(source='actor.username', read_only=True)

    class Meta:
        model = FotoProvenanceEvent
        fields = [
            'id',
            'actor_username',
            'action',
...
class FotoSerializer(serializers.ModelSerializer):
        ...
        try:
            ...
            for pv in obj.variant_assets.all():
                if not pv.image or not getattr(pv.image, 'name', ''):
                    continue
                if pv.kind == FotoVariant.KIND_FEED and feed_url is None:
                    feed_url = _variant_url(pv.image)
                elif pv.kind == FotoVariant.KIND_SQUARE and square_url is None:
    ...
    def get_story_display_image_url(self, obj):
        ...
        try:
            for pv in obj.variant_assets.all():
                if pv.kind == FotoVariant.KIND_STORY and pv.image and getattr(pv.image, 'name', ''):
    ...
    def validate(self, attrs):
        ...
        if method == 'POST':
            ...
            if vid and is_story:
                raise serializers.ValidationError({
                    'story_video': 'Les fotos au format story ne peuvent pas inclure de vidéo (image uniquement).',
                })
        ...
        if method in ('PUT', 'PATCH') and self.instance:
            foto = self.instance
            merged_story = attrs.get('is_story', foto.is_story)
            ...
            if merged_story and new_video:
                raise serializers.ValidationError({
                    'story_video': ['Les fotos story ne peuvent pas inclure de vidéo.'],
                })
            if merged_story and foto.story_video and getattr(pin.story_video, 'name', '') and vid_up is serializers.empty:
                raise serializers.ValidationError({
                    'is_story': [
                        'Ce foto contient encore une vidéo : retirez-la (remplacez le média) avant d’activer le format story.',
                    ],
                })
        return attrs
    ...
    def get_is_liked(self, obj):
        ...
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            return Like.objects.filter(user=request.user, foto=obj).exists()
        return False
    ...
    def get_is_saved(self, obj):
        ...
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            return Save.objects.filter(user=request.user, foto=obj).exists()
        return False
    ...
    def get_viewer_has_reported(self, obj):
        ...
        if not request or not request.user.is_authenticated:
            return False
        return ContentReport.objects.filter(reporter=request.user, foto=obj).exists()
    ...
    def get_is_boosted(self, obj):
        ...
        now = timezone.now()
        return FotoBoost.objects.filter(
            foto=obj,
    ...
    def _apply_foto_tags_and_boards(self, foto, request, private_tags, public_tags, board_ids):
        ...
        if merged_tags:
            ...
            for tag in merged_tags:
                hashtag, _ = Hashtag.objects.get_or_create(name=tag)
                hashtag_objs.append(hashtag)
            foto.hashtags.set(hashtag_objs)
    ...
    def create(self, validated_data):
        ...
        if request and request.user.is_authenticated:
            ...
            apply_foto_creation_rate_limits(request.user.id, story_flag)
            enforce_identical_content_flood(
                request.user.id,
                'pin',
                pin_body_fingerprint(
        ...
        with transaction.atomic():
            foto = super().create(validated_data)
            if request and request.user.is_authenticated:
                apply_server_visual_moderation_to_pin(pin, request.user.profile)
            self._apply_foto_tags_and_boards(pin, request, private_tags, public_tags, board_ids)
        return foto
    ...
    def update(self, instance, validated_data):
        ...
        if 'topic' in validated_data:
            ...
            validated_data['topic'] = self._resolve_topic(topic_value)
        with transaction.atomic():
            foto = super().update(instance, validated_data)
            request = self.context.get('request')
            if not request or request.user != foto.author:
                return foto
...
class StandaloneStoryCreateSerializer(serializers.Serializer):
    """POST minimal story Plus/Pro sans foto persistant en grille après 24h."""
...
class BoardDetailSerializer(BoardSerializer):
    ...
    class Meta(BoardSerializer.Meta):
        fields = list(BoardSerializer.Meta.fields) + ['fotos', 'viewer_can_manage', 'share_token']
