...
Production : Celery Beat `fotos-send-weekly-pro-digest` (lun 9h UTC).
...
from fotos.weekly_stats import pro_weekly_views_stats
from notifications.notification_i18n import create_localized_notification


class Command(BaseCommand):
    help = 'Envoie digest hebdomadaire Pro : fotos les plus vus (FotoViewEvent 7 derniers jours).'
    ...
    def handle(self, *args, **options):
        ...
        for profile in qs:
            ...
            if lang.startswith('fr'):
                ...
                lines_body.append('Here are your fotos with the most views in the last 7 days:')
                lines_body.append('')
            for i, foto in enumerate(top, 1):
                vw = int(getattr(pin, 'views_week', 0))
                pin_url = f'{base_url}/foto/{pin.slug}' if base_url else f'/foto/{pin.slug}'
            ...
            create_localized_notification(
                ...
                title_fr='Vos fotos les plus vus cette semaine',
                message_fr=digest_message_fr,
                action_url='/creator',
                metadata={'digest': 'weekly_views', 'days': 7},
            )

            subject_fr = '[Fotoce Pro] Ton récap’ de la semaine'
            subject_en = '[Fotoce Pro] Your weekly recap'
