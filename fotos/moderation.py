...
def _consume_fixed_window(user_id: int, bucket: str, limit: int, window_seconds: int) -> None:
    ...
    key = f'fotoce_rl:{bucket}:{user_id}:{slot}'
    n = cache.get(key, 0)
    if n >= limit:
...
def _consume_fixed_window_ip(ident: str, bucket: str, limit: int, window_seconds: int) -> None:
    ...
    if not safe:
        return
    slot = int(time.time()) // window_seconds
    key = f'fotoce_rl:{bucket}:{safe}:{slot}'
    n = cache.get(key, 0)
    if n >= limit:
        raise serializers.ValidationError({
            'non_field_errors': [MSG_RATE],
...
def apply_foto_creation_rate_limits(user_id: int, is_story: bool) -> None:
...
def enforce_identical_content_flood(user_id: int, kind: str, fingerprint: str) -> None:
    ...
    key = f'fotoce_flood:{kind}:{user_id}:{fingerprint}:{slot}'
    n = cache.get(key, 0)
...
def apply_foto_report_thresholds(foto) -> None:
    from .models import Foto

    updates = {}
    if foto.report_count >= REPORTS_HIDE:
        updates['moderation_hidden'] = True
        updates['needs_review'] = True
    elif foto.report_count >= REPORTS_NEED_REVIEW:
        updates['needs_review'] = True
    if updates:
        Foto.objects.filter(pk=pin.pk).update(**updates)
...
def increment_foto_reports(foto_id: int) -> None:
    from .models import Foto

    Foto.objects.filter(pk=foto_id).update(report_count=F('report_count') + 1)
