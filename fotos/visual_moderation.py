...
def classify_foto_visual(pin, verified_adult: bool) -> VisualModerationDecision | None:
    """
...
def apply_server_visual_moderation_to_pin(pin, profile) -> None:
    ...
    try:
        decision = classify_foto_visual(pin, adult)
    except Exception as exc:
    ...
    Foto.objects.filter(pk=pin.pk).update(
        media_sensitive_blur=blur,
        needs_review=needs,
    )
    foto.media_sensitive_blur = blur
    foto.needs_review = needs
