...
from fotos.feed_cache import get_home_feed_version
...
class FeedCacheTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        for i in range(12):
            _make_public_pin(cls.author, f'Cache Foto {i}')
    ...
    def test_home_feed_page1_cache_hit(self):
        r1 = self.client.get('/api/fotos/home-feed/?page=1&page_size=10')
        self.assertEqual(r1.status_code, 200)
        self.assertEqual(r1['X-Cache'], 'MISS')

        r2 = self.client.get('/api/fotos/home-feed/?page=1&page_size=10')
        self.assertEqual(r2.status_code, 200)
        self.assertEqual(r2['X-Cache'], 'HIT')
        self.assertEqual(len(r2.data['results']), 10)

    def test_home_feed_invalidated_on_new_foto_from_following(self):
        self.client.get('/api/fotos/home-feed/?page=1&page_size=10')
        ver_before = get_home_feed_version(self.viewer.id)

        _make_public_pin(self.author, 'Fresh Foto Burst')

        ver_after = get_home_feed_version(self.viewer.id)
        self.assertGreater(ver_after, ver_before)

        r3 = self.client.get('/api/fotos/home-feed/?page=1&page_size=10')
        self.assertEqual(r3['X-Cache'], 'MISS')

    def test_discover_page1_global_cache(self):
        client = APIClient()
        r1 = client.get('/api/fotos/discover/?page=1&page_size=10')
        self.assertEqual(r1.status_code, 200)
        self.assertEqual(r1['X-Cache'], 'MISS')

        r2 = client.get('/api/fotos/discover/?page=1&page_size=10')
        self.assertEqual(r2['X-Cache'], 'HIT')
    ...
    def test_creator_stats_versioned_cache(self):
        ...
        r1 = self.client.get('/api/fotos/creator-stats/?totals_only=1')
        self.assertEqual(r1.status_code, 200)
        self.assertEqual(r1['X-Cache'], 'MISS')

        r2 = self.client.get('/api/fotos/creator-stats/?totals_only=1')
        self.assertEqual(r2['X-Cache'], 'HIT')

        from fotos.feed_cache import get_creator_stats_version
        ...
        r3 = self.client.get('/api/fotos/creator-stats/?totals_only=1')
        self.assertEqual(r3['X-Cache'], 'MISS')

    def test_no_cache_bypass(self):
        self.client.get('/api/fotos/home-feed/?page=1&page_size=10')
        r = self.client.get('/api/fotos/home-feed/?page=1&page_size=10&no_cache=1')
    ...
    def test_public_endpoints_allow_invalid_bearer(self):
        ...
        for path in (
            '/api/fotos/discover/?page=1&page_size=5',
            '/api/fotos/topics/?limit=5',
            '/api/fotos/explore-boards/?page=1&page_size=5',
        ):
