"""Hub créateur Pro : fotos récents et file de modération commentaires."""
from __future__ import annotations

from math import ceil

from .models import Comment, Foto
...
def paginated_recent_pins(request, user, page: int, page_size: int):
    ...
    qs = Foto.objects.filter(author=user).order_by('-created_at')
    total = qs.count()
    total_pages = max(1, ceil(total / page_size)) if total else 1
    offset = (page - 1) * page_size
    fotos = qs[offset : offset + page_size]
    rows = []
    for p in pins:
        rows.append(
            {
...
def paginated_comment_inbox(user, limit: int, offset: int):
    """
    Derniers commentaires sur les fotos de l’utilisateur (file modération).
