...
class Foto(models.Model):
    ...
    author = models.ForeignKey(User, on_delete=models.CASCADE, related_name='fotos')
    topic = models.ForeignKey(Topic, on_delete=models.SET_NULL, null=True, blank=True, related_name='fotos')
    ...
    boards = models.ManyToManyField(Board, blank=True, related_name='fotos', through='FotoBoard')
    hashtags = models.ManyToManyField(Hashtag, blank=True, related_name='fotos')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    # Plus/Pro « story éphémère » : après 24h la ligne est détruite (pas d’archive en foto grille).
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name='unique_foto_upload_idempotency_per_author',
            ),
        ]
        indexes = [
            models.Index(fields=['-created_at'], name='fotos_foto_created_desc_idx'),
            models.Index(fields=['author', '-created_at'], name='fotos_foto_author_created_idx'),
            models.Index(fields=['topic', '-created_at'], name='fotos_foto_topic_created_idx'),
        ]
    ...
    def refresh_story_expiry(self):
        ...
        # Un foto classique en format story reste archivé comme un foto normal (pas de 24h).
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            try:
                prev = Foto.objects.only('image', 'story_video').get(pk=self.pk)
        ...
        if not self.slug:
            ...
            count = 1
            while Foto.objects.filter(slug=self.slug).exists():
...
class FotoVariant(models.Model):
    """Crops / ratios alternatifs pour un même foto (story 9:16, carré, paysage). Une ligne métier unique."""
    KIND_STORY = 'story'
    KIND_SQUARE = 'square'
    ...
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='variant_assets')
    kind = models.CharField(max_length=20, choices=KIND_CHOICES)
    image = models.ImageField(upload_to='fotos/variants/')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f'{self.foto_id}:{self.kind}'

    def save(self, *args, **kwargs):
        if self.pk:
            try:
                old = FotoVariant.objects.only('image').get(pk=self.pk)
            except FotoVariant.DoesNotExist:
                old = None
            if old is not None:
...
class FotoBoard(models.Model):
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='foto_board_memberships')
    board = models.ForeignKey(Board, on_delete=models.CASCADE, related_name='foto_board_memberships')
    ...
    def __str__(self):
        return f'{self.foto_id}->{self.board_id}@{self.position}'


class Save(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='saves')
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='saves')
    created_at = models.DateTimeField(auto_now_add=True)
...
class Like(models.Model):
    ...
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='likes')
    created_at = models.DateTimeField(auto_now_add=True)
...
class Comment(models.Model):
    ...
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='comments')
...
class ContentReport(models.Model):
    """Signalement utilisateur (profil, foto / story, commentaire)."""

    reporter = models.ForeignKey(User, on_delete=models.CASCADE, related_name='content_reports')
    foto = models.ForeignKey(Foto, null=True, blank=True, on_delete=models.CASCADE, related_name='reports')
    ...
    class Meta:
        ordering = ['-created_at']
        indexes = [
            models.Index(fields=['foto', '-created_at']),
    ...
    def __str__(self):
        if self.foto_id:
            return f'report foto {self.foto_id} by {self.reporter_id}'
        if self.comment_id:
...
class PrivatePinTag(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='private_foto_tags')
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='private_tags')
...
class FotoProvenanceEvent(models.Model):
    ...
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='provenance_events')
    ...
    def __str__(self):
        return f"{self.foto_id}:{self.action}:{self.current_hash[:12]}"
...
class FotoEmbedding(models.Model):
    foto = models.OneToOneField(Foto, on_delete=models.CASCADE, related_name='embedding_profile')
...
class UserInteraction(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='foto_interactions')
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='user_interactions', null=True, blank=True)
...
class TagInvisible(models.Model):
    foto = models.ForeignKey(Foto, on_delete=models.CASCADE, related_name='invisible_tags')
    tag = models.CharField(max_length=100)
