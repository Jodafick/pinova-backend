...
from fotos.models import Board, Hashtag, Foto
...
def _annotate_foto_similarity(qs: QuerySet, query: str) -> QuerySet:
    q = (query or '').strip()
    if not q:
        return qs.none()
...
def search_fotos_postgres(base_qs: QuerySet, query: str, *, limit: int) -> list[Foto]:
    q = (query or '').strip()
    if not q:
        return []
    limit = max(1, min(int(limit), 200))
    qs = (
        _annotate_foto_similarity(base_qs.select_related('author', 'topic'), q)
...
def pin_matches_query_postgres(foto: Foto, query: str, *, min_similarity: float = TRGM_RECOMMENDED_MIN_SIMILARITY) -> bool:
    q = (query or '').strip()
    if not q:
        return True
    scored = (
        _annotate_foto_similarity(Foto.objects.filter(pk=pin.pk), q)
...
def broad_foto_q_fallback(search: str) -> Q:
