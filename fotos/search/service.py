...
logger = logging.getLogger('fotoce.search')
...
def search_pins(
    ...
    *,
    ...
    if _use_typesense():
        try:
            return _search_fotos_typesense(
                base_qs,
                q,
                limit=limit,
            ...
            logger.warning('Typesense fotos search failed, fallback postgres: %s', exc)

    if postgres.postgres_trgm_enabled():
        return postgres.search_fotos_postgres(base_qs, q, limit=limit)

    return _search_fotos_legacy(base_qs, q, limit=limit)


def _search_fotos_typesense(
    base_qs: QuerySet,
    query: str,
    *,
    limit: int,
    ...
    ids = typesense_client.search_foto_ids(query, limit=limit * 3, exclude_author_id=exclude_author_id)
    if not ids:
        return []
...
def _search_fotos_legacy(base_qs: QuerySet, query: str, *, limit: int) -> list[Foto]:
    candidates = list(base_qs.filter(broad_foto_q(query)).select_related('author', 'topic')[:150])
    candidates.sort(
...
def pin_matches_query(foto: Foto, query: str, *, min_score: float = 0.22) -> bool:
    q = (query or '').strip()
    if not q:
        return True
    if postgres.postgres_trgm_enabled():
    ...
    return fuzzy_score(q, foto.title or '', foto.description or '', foto.author.username) >= min_score
...
def discover_pins_filter(base_qs: QuerySet, query: str) -> QuerySet:
    ...
    if _use_typesense():
        try:
            fotos = _search_fotos_typesense(base_qs, q, limit=200, exclude_author_id=None)
    ...
    if postgres.postgres_trgm_enabled():
        fotos = postgres.search_fotos_postgres(base_qs, q, limit=200)
        ids = [p.pk for p in pins]
        if not ids:
            return base_qs.none()
        order = Case(*[When(pk=pk, then=Value(pos)) for pos, pk in enumerate(ids)], output_field=IntegerField())
        return base_qs.filter(pk__in=ids).annotate(_discover_sort=order).order_by('_discover_sort')
    return base_qs.filter(broad_foto_q(q))
