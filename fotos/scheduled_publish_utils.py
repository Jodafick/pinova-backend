...
def publish_due_scheduled_pins(queryset, *, limit: int = 1000) -> int:
    """
    Pour chaque foto dû dans ce queryset : notification à l'auteur (post_save push),
    puis scheduled_publish_at = None.

    select_for_update n'est pas utilisé (SQLite / compatibilité).

    Retourne le nombre de fotos traités.
    """
    limit = max(1, limit)
    with transaction.atomic():
        ...
        Foto.objects.filter(pk__in=cleared_ids).update(scheduled_publish_at=None)
...
def publish_user_due_scheduled_pins(user, *, limit: int = 50) -> int:
    ...
    if not getattr(user, 'pk', None) or not getattr(user, 'is_authenticated', False):
        return 0
    return publish_due_scheduled_pins(Foto.objects.filter(author=user), limit=limit)
