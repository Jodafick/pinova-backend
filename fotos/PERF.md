# Performance — fils de fotos (feed)

Pattern pour servir une page de **10 pins** en **≤ 5 requêtes SQL** (hors auth/session).

## Budget requêtes (page feed 10 fotos)

| # | Requête | Rôle |
|---|---------|------|
| 1 | `COUNT(*)` | Pagination DRF (`PinFeedPagination`) |
| 2 | `SELECT fotos …` | Pins + `select_related(author, topic)` + annotations viewer |
| 3 | Prefetch `hashtags` | M2M hashtags |
| 4 | Prefetch `foto_board_memberships` + `board` | Boards ordonnés par position |
| 5 | Prefetch `variant_assets` | Variantes story / ratios |

La **sérialisation** (`PinSerializer`) ne doit ajouter **aucune** requête si le queryset passe par `optimize_foto_feed_queryset`.

## Module `feed_queryset.py`

```python
from fotos.feed_queryset import optimize_foto_feed_queryset, feed_serializer_context

qs = FotoViewSet().get_queryset()
qs = optimize_foto_feed_queryset(qs, request)
page = list(qs[:10])
...
`get_boards()` utilise `obj.foto_board_memberships.all()` (cache prefetch), plus `FotoBoard.objects.filter(pin=obj)`.
...
1. `active_boosted_foto_ids()` — **1 requête** pour tous les boosts actifs
2. Passe `boosted_foto_ids` à `recommendation_score` (plus de `PinBoost.objects.filter(pin=…).exists()` par foto)
3. Applique `annotate_foto_feed` + `prefetch_foto_feed_relations` avant le tri Python

## Intégration ViewSet

Actions feed : `list`, `discover`, `following`, `home_feed`, `recommendations`, `header_search`.

- `get_queryset()` : pas de prefetch générique sur ces actions (délégué à `optimize_foto_feed_queryset`)
...
```bash
python manage.py test fotos.tests_feed_perf -v 2
```
...
Objectif prod : **p95 < 200 ms** page 1 (10 fotos) avec ce pattern + indexes `(created_at)`, `(author_id, created_at)` (voir migration `0006_foto_feed_indexes`).

## Cache Redis (feed + stats)

Module `fotos/feed_cache.py` — TTL et clés versionnées :

| Scope | TTL | Clé | Invalidation |
|-------|-----|-----|--------------|
| `home_feed` page 1 | 60 s | `fotoce:feed:home:v{ver}:{user_id}:p1:…` | Bump `ver` abonnés quand un auteur suivi publie |
| `discover` page 1 global | 120 s | `fotoce:feed:discover:g{gver}:global:p1:…` | Bump `gver` à chaque foto public |
| `discover` page 1 user | 120 s | `fotoce:feed:discover:g{gver}:u{uver}:{user_id}:…` | Bump `uver` / `gver` |
| `creator_stats` | 90 s | `fotoce:creator_stats:v{ver}:{user_id}:…` | Bump `ver` à chaque foto auteur |

- Header debug : **`X-Cache: HIT|MISS`**
- Bypass : `?no_cache=1`
- Logs JSON : logger `fotoce.cache` (`event`, `cache_scope`, `cache_key`, `user_id`)
...
```bash
python manage.py test fotos.tests_feed_cache -v 2
```
