...
def fetch_pins_for_slots(following_qs, discover_qs, slots: list[SourceSlot]):
    """Charge les fotos pour une page via au plus deux requêtes DB (tranches contiguës)."""
    if not slots:
        return []
    ...
    if following_indices:
        ...
        for offset, foto in enumerate(following_qs[lo : hi + 1], start=lo):
            following_map[offset] = foto

    discover_map: dict[int, object] = {}
    if discover_indices:
        lo, hi = min(discover_indices), max(discover_indices)
        for offset, foto in enumerate(discover_qs[lo : hi + 1], start=lo):
            discover_map[offset] = foto

    page_fotos = []
    for src, idx in slots:
        foto = following_map.get(idx) if src == 'following' else discover_map.get(idx)
        if foto is not None:
            page_fotos.append(foto)
    return page_fotos


def interleave_foto_lists(following_items, discover_items):
