"""Tests unitaires de la politique mot de passe Pinova."""
...
from accounts.password_policy import (
    ...
    PinovaPasswordValidator,
    evaluate_pinova_password,
    get_password_rules_payload,
    validate_pinova_password,
)


class PinovaPasswordPolicyUnitTests(SimpleTestCase):
    def test_valid_password_passes(self):
        self.assertEqual(evaluate_pinova_password('Pinova2026'), [])
        validate_pinova_password('Pinova2026')

    def test_too_short_fails(self):
        failed = evaluate_pinova_password('Ab1')
        self.assertIn('min_length', failed)

    def test_missing_letter_fails(self):
        failed = evaluate_pinova_password('12345678')
        self.assertIn('has_letter', failed)

    def test_missing_digit_fails(self):
        failed = evaluate_pinova_password('Password')
        self.assertIn('has_digit', failed)

    def test_trivial_list_fails(self):
        failed = evaluate_pinova_password('password123')
        self.assertIn('not_trivial', failed)

    def test_email_local_part_fails(self):
        failed = evaluate_pinova_password('jean1234', email='jean@example.com')
        self.assertIn('not_trivial', failed)

    def test_username_fails(self):
        failed = evaluate_pinova_password('creator99', username='creator')
        self.assertIn('not_trivial', failed)

    def test_validator_raises_one_message_per_rule(self):
        validator = PinovaPasswordValidator()
...
class PasswordRulesApiTests(TestCase):
    ...
    def test_get_password_rules_en_header(self):
        r = self.client.get('/api/auth/password-rules/', HTTP_X_PINOVA_LANG='en')
        self.assertEqual(r.status_code, 200)
