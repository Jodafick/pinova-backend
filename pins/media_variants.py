...
from .models import Pin, PinVariant
from .storage_media import unlink_field_file

logger = logging.getLogger('pinova.media.variants')
...
def ensure_pin_feed_thumbnail(pin: Pin, *, force: bool = False) -> PinVariant | None:
    """
    ...
    if Image is None:
        return None
    if not pin.image or not getattr(pin.image, 'name', ''):
        return None

    existing = (
        PinVariant.objects.filter(pin=pin, kind=PinVariant.KIND_FEED)
        .select_related('pin')
        .first()
    )
    if existing and not force:
        try:
            src_mtime = pin.image.storage.get_modified_time(pin.image.name).timestamp()
    ...
    try:
        with pin.image.open('rb') as fh:
            im = Image.open(fh)
            im.load()
        payload = _encode_feed_jpeg(im)
    except Exception as exc:
        logger.warning('feed_thumbnail_failed pin=%s: %s', pin.pk, exc)
        return None
    ...
    with transaction.atomic():
        variant = existing
        if variant is None:
            variant = PinVariant(pin=pin, kind=PinVariant.KIND_FEED)
