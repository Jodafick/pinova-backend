...
from pins.models import Board, Hashtag, Pin, Topic
from pins.search.service import (
...
class SearchUtilsTests(TestCase):
    ...
    def test_broad_pin_q_matches_title_token(self):
        author = User.objects.create_user('alice', password='x')
        pin = Pin.objects.create(title='Minimal salon design', author=author, slug='minimal-salon')
        qs = Pin.objects.filter(broad_pin_q('salon'))
...
class LegacySearchServiceTests(TestCase):
    ...
    def test_search_pins_legacy_path(self):
        qs = Pin.objects.filter(visibility=Pin.VISIBILITY_PUBLIC)
    ...
    def test_pin_matches_query_legacy(self):
...
class TypesenseFallbackTests(TestCase):
    def setUp(self):
        ...
        self.pin = Pin.objects.create(title='Typesense pin', author=self.author, slug='typesense-pin')

    @mock.patch('pins.search.typesense_client.search_pin_ids', side_effect=RuntimeError('down'))
    @mock.patch('pins.search.tasks.sync_user_typesense')
    def test_search_pins_falls_back_when_typesense_down(self, _mock_sync, _mock_search):
        with override_settings(SEARCH_USE_TRIGRAM=False):
            results = search_pins(Pin.objects.all(), 'typesense', limit=5)
        self.assertTrue(any(p.pk == self.pin.pk for p in results))

    @mock.patch('pins.signals_search.sync_pin_typesense.delay')
    def test_typesense_sync_lag_does_not_block_http_search(self, mock_delay):
        ...
        pin = Pin.objects.create(title='Lag sync pin', author=self.author, slug='lag-sync-pin')
        mock_delay.assert_called()
        with mock.patch('pins.search.typesense_client.search_pin_ids', side_effect=RuntimeError('down')):
            with override_settings(SEARCH_USE_TRIGRAM=False):
                results = search_pins(Pin.objects.filter(pk=pin.pk), 'lag', limit=5)
        self.assertTrue(any(p.pk == pin.pk for p in results))
...
class PostgresTrigramTests(TestCase):
    ...
    def test_search_pins_postgres_trgm(self):
        qs = Pin.objects.filter(visibility=Pin.VISIBILITY_PUBLIC)
