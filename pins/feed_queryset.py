...
from .models import Comment, ContentReport, Like, Pin, PinBoard, Save
...
def _relation_count_subquery(model, *, pin_field='pin_id'):
    return Subquery(
...
def annotate_pin_feed(queryset, user):
    """
    ...
    if user and user.is_authenticated:
        ...
        extra: dict = {
            '_is_liked': Exists(Like.objects.filter(user_id=user.id, pin_id=OuterRef('pk'))),
            '_is_saved': Exists(Save.objects.filter(user_id=user.id, pin_id=OuterRef('pk'))),
            '_is_boosted': Exists(
            ...
            '_can_comment': Case(
                ...
                When(comments_policy=Pin.COMMENTS_CLOSED, then=Value(False)),
                When(comments_policy=Pin.COMMENTS_OPEN, then=Value(True)),
                When(comments_policy=Pin.COMMENTS_FOLLOWERS_ONLY, then=follower_exists),
                default=Value(False),
...
def prefetch_pin_feed_relations(queryset):
    """PinBoard + hashtags + variant_assets en une passe prefetch."""
...
def optimize_pin_feed_queryset(queryset, request):
    """Applique annotations + prefetch pour les endpoints feed."""
    user = request.user if request.user.is_authenticated else None
    if user is not None:
        cache_viewer_following_profile_ids(request)
    queryset = annotate_pin_feed(queryset, user)
    return prefetch_pin_feed_relations(queryset)


def feed_serializer_context(request, base_context: dict | None = None) -> dict:
    """Contexte PinSerializer feed : following pré-chargé pour is_following."""
