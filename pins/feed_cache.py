...
DISCOVER_GLOBAL_VER_KEY = 'pinova:feed:discover:global_ver'
...
def _home_feed_version_key(user_id: int) -> str:
    return f'pinova:feed:home:ver:{user_id}'
...
def home_feed_page1_key(user_id: int, *, topic: str, page_size: int, reco_on: bool) -> str:
    ...
    return (
        f'pinova:feed:home:v{ver}:{user_id}:p1:'
...
def _discover_user_version_key(user_id: int) -> str:
    return f'pinova:feed:discover:user_ver:{user_id}'
...
def discover_page1_global_key(*, topic: str, page_size: int) -> str:
    ...
    return (
        f'pinova:feed:discover:g{gver}:global:p1:'
...
def discover_page1_user_key(user_id: int, *, topic: str, page_size: int) -> str:
    ...
    return (
        f'pinova:feed:discover:g{gver}:u{uver}:{user_id}:p1:'
        f'{_topic_token(topic)}:ps{_page_size_token(page_size)}'
    )
...
def _creator_stats_version_key(user_id: int) -> str:
    return f'pinova:creator_stats:ver:{user_id}'
...
def creator_stats_cache_key(
    ...
    *,
    ...
    return (
        f'pinova:creator_stats:v{ver}:{user_id}:'
...
def invalidate_on_public_pin_change(*, author_id: int, invalidate_discover: bool = True) -> None:
...
def warm_discover_page1_cache(*, topic: str = '', page_size: int = 10) -> str:
    ...
    path = f'/api/pins/discover/?page=1&page_size={page_size}'
    if topic:
        path += f'&topic={topic}'
