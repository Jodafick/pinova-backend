...
from pins.media_variants import FEED_THUMB_MAX_SIDE, ensure_pin_feed_thumbnail
from pins.models import Pin, PinVariant, Topic
...
class FeedThumbnailTests(TestCase):
    ...
    def test_creates_feed_variant_max_400px(self):
        ...
        variant = ensure_pin_feed_thumbnail(pin)
        self.assertIsNotNone(variant)
        self.assertEqual(variant.kind, PinVariant.KIND_FEED)
    ...
    def test_signal_on_create(self):
        ...
        self.assertTrue(
            PinVariant.objects.filter(pin=pin, kind=PinVariant.KIND_FEED).exists()
        )
