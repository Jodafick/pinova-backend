...
class Command(BaseCommand):
    help = 'Réindexe pins, users et boards dans Typesense (SEARCH_ENGINE=typesense).'
    ...
    def handle(self, *args, **options):
        ...
        pin_qs = Pin.objects.select_related('author__profile').prefetch_related('hashtags').order_by('pk')
        total_pins = 0
        batch: list[dict] = []
        for pin in pin_qs.iterator(chunk_size=batch_size):
            if documents.pin_searchable(pin):
                batch.append(documents.pin_to_typesense_doc(pin))
            if len(batch) >= batch_size:
                typesense_client.get_typesense_client().collections[PINS_COLLECTION].documents.import_(
                    batch, {'action': 'upsert'}
                )
