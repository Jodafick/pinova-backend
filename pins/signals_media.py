...
from .models import Comment, Pin, PinVariant
from .storage_media import unlink_field_file


@receiver(pre_delete, sender=PinVariant)
...
def _purge_pin_main_media(sender, instance, **kwargs):
    # Variants déjà pré-supprimées (récepteurs pré_delete enfants puis cascade) :
    # ici uniquement médias du pin principal.
    unlink_field_file(instance.image)
